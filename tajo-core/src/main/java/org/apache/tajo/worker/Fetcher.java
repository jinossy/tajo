...
public class Fetcher {
  ...
  public FileChunk get() throws IOException {
    ...
    try {
      ...
      if(elapsedTime > 1000) {
        long bytePerSec = ((fileChunk.length() * 1000) / elapsedTime) / 1000;
      ...
      LOG.info(String.format("Fetcher :%d ms elapsed. %s/sec, state:%s, URL:%s",
          elapsedTime, transferSpeed, getState(), uri));
    }
  }
